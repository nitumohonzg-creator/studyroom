...
let openAuthorFolders = [], allCurrentQuestions = [], wrongQuestions = JSON.parse(localStorage.getItem('studyRoomWrong')) || [], allPublicRooms = []; 

updateRevisionCount(); checkDailyStreak(); 

function checkDailyStreak() {
  const today = new Date().toDateString(); let streakData = JSON.parse(localStorage.getItem('studyRoomStreak')) || { date: '', count: 0, streak: 0 };
  if (streakData.date !== today) {
    let yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
    if (streakData.date === yesterday.toDateString() && streakData.count >= 10) {} else if (streakData.date !== yesterday.toDateString()) { streakData.streak = 0; }
    streakData.date = today; streakData.count = 0; localStorage.setItem('studyRoomStreak', JSON.stringify(streakData));
  } updateStreakUI();
}
...
// 🌟 UPDATE 3.5: Modified for "Save & Next"
function openManualAddBox() { 
  ...
  // Show Next/Finish buttons, hide Update button
  document.getElementById('addMoreBtnContainer').style.display = 'flex';
...
function hideAllScreens() { ['authBox','dashboardScreen','discoverScreen','revisionScreen','profileScreen','roomViewScreen'].forEach(id => { let el = document.getElementById(id); if(el) el.style.display = 'none'; }); closeProfileMenuModal(); closeRoomMenuModal(); closeAddMcqChoiceModal(); }
...
auth.onAuthStateChanged((user) => { if (user) { db.collection('users').doc(user.uid).get().then((doc) => { let p = localStorage.getItem('pendingJoin'); if (p) { localStorage.removeItem('pendingJoin'); document.getElementById('joinRoomIdInput').value = p; joinRoomByFirebase(); } else { handleHashChange(); } }); } else { window.location.hash = ''; handleHashChange(); } });

function openLeaderboard() {
  let m = document.getElementById('leaderboardModal'), c = document.getElementById('leaderboardList'); if(!m || !c) return; m.style.display = 'flex'; c.innerHTML = "Loading...";
  db.collection('rooms').doc(currentRoomId).collection('leaderboard').orderBy('score', 'desc').limit(10).get().then(snap => {
      if(snap.empty) { c.innerHTML = "No scores yet."; return; }
      let html = '', rank = 1; snap.forEach(doc => { let d = doc.data(), medal = rank===1?'🥇':(rank===2?'🥈':(rank===3?'🥉':'🏅')); html += `<div style="display:flex; justify-content:space-between; padding:10px 5px; border-bottom:1px solid var(--border-color);"><span>${medal} <b>${d.name||'User'}</b></span><span style="color:#28a745; font-weight:bold;">${d.score||0} pts</span></div>`; rank++; }); c.innerHTML = html;
...
function signupUser() { const n=document.getElementById('signupName').value.trim(), e=document.getElementById('signupEmail').value.trim(), p=document.getElementById('signupPassword').value.trim(); if(!n||!e||!p) return alert("Fill all fields"); auth.createUserWithEmailAndPassword(e, p).then(c => db.collection('users').doc(c.user.uid).set({ displayName: n, email: e, username: "", mobile: "", bio: "" })).then(() => { alert("Created!"); toggleAuth('login'); }).catch(e => alert(e.message)); }
...
function forgotPassword() { const e=document.getElementById('loginEmail').value.trim(); if(!e) return alert("Enter email!"); auth.sendPasswordResetEmail(e).then(() => alert("Link sent!")).catch(e => alert(e.message)); }
...
function loadDiscoverRooms() {
  ...
  db.collection('rooms').where('isPublic', '==', true).get().then(snap => {
    allPublicRooms = []; if(snap.empty) { c.innerHTML = '<p>No public rooms.</p>'; return; } let html = '';
    snap.forEach(doc => { let r=doc.data(); r.id=doc.id; allPublicRooms.push(r); let im = r.members && auth.currentUser && r.members.includes(auth.currentUser.uid); let btn = im ? `<button class="btn" style="width:auto; padding:5px 10px; background:#6c757d; font-size:11px;" disabled>Joined</button>` : `<button class="btn" style="width:auto; padding:5px 10px; background:#28a745; font-size:11px;" onclick="joinSpecificRoom('${r.id}')">Join</button>`; html += `<div id="pub-room-${r.id}" style="display:flex; justify-content:space-between; align-items:center; padding:15px; border:1px solid var(--border-color); border-radius:8px; margin-bottom:10px; background:var(--card-bg);"><div><b style="color:var(--primary-btn); font-size:16px;">${r.roomName||'Room'}</b><br><span style="font-size:11px; color:var(--text-color);">By: ${r.creatorName||'Admin'} | 👥 ${r.members?r.members.length:1} Members</span></div>${btn}</div>`; }); c.innerHTML = html;
  }).catch(e => { c.innerHTML = `❌ Error`; });
}
...
function saveRoomToFirebase() {
  const n = document.getElementById('newRoomName').value.trim(), t = document.getElementById('isRoomPublicToggle'), p = t ? t.checked : false; if(!n) return alert("Enter Name!");
  db.collection('users').doc(auth.currentUser.uid).get().then(doc => { db.collection('rooms').add({ roomName: n, creatorId: auth.currentUser.uid, creatorName: doc.data().displayName, admins: [auth.currentUser.uid], members: [auth.currentUser.uid], adminOnlyMCQ: false, isPublic: p, createdAt: firebase.firestore.FieldValue.serverTimestamp() }).then(() => { alert("Created!"); closeCreateRoom(); loadMyRooms(); }); });
}
...
function loadMyRooms() {
  ...
  db.collection('rooms').where("members", "array-contains", auth.currentUser.uid).get().then(snap => {
    if(snap.empty) { c.innerHTML = '<p style="font-size:13px;">No rooms found.</p>'; return; } let html = '';
    snap.forEach(doc => { let p = doc.data().isPublic ? '<span style="color:#17a2b8; font-size:10px;">🌍 Public</span>' : '<span style="color:#6c757d; font-size:10px;">🔒 Private</span>'; html += `<div class="q-card" style="display:flex; justify-content:space-between; align-items:center;"><div><b style="font-size:16px;">${doc.data().roomName}</b><br>${p}</div><button class="btn" style="width:auto; padding:6px 12px; margin:0;" onclick="enterRoom('${doc.id}', '${doc.data().roomName}')">Enter</button></div>`; }); c.innerHTML = html;
  });
}

function enterRoom(id, name) {
  currentRoomId = id; currentRoomName = name; document.getElementById('roomTitleText').innerText = name; document.getElementById('searchQuestion').value = ''; closeRoomMenuModal(); closeAddMcqChoiceModal(); openAuthorFolders = [];
  db.collection('rooms').doc(id).get().then(doc => {
    if(doc.exists) {
      ...
      let db = document.getElementById('deleteRoomMenuBtn'); if(db) db.style.display = (auth.currentUser.uid === currentRoomCreator) ? 'block' : 'none';
...
function saveRoomEdit() {
  let n = document.getElementById('editRoomNameInput').value.trim(), a = document.getElementById('editRoomAdminOnlyToggle').checked, p = document.getElementById('editRoomPublicToggle').checked; if(!n) return alert("Enter valid name");
  db.collection('rooms').doc(currentRoomId).update({ roomName: n, adminOnlyMCQ: a, isPublic: p }).then(() => { alert("Updated!"); currentRoomName=n; currentRoomAdminOnlyMCQ=a; currentRoomIsPublic=p; document.getElementById('roomTitleText').innerText=n; closeEditRoomBox(); let ab=document.getElementById('addMcqMainBtn'); if(ab) ab.style.display=(a && !currentRoomAdmins.includes(auth.currentUser.uid))?'none':'block'; });
}

function loadRoomMembers() {
  const c = document.getElementById('membersListContainer'); if(!c) return; c.innerHTML = 'Loading...';
  db.collection('rooms').doc(currentRoomId).get().then(doc => {
    if(!doc.exists) return; const m = doc.data().members || []; currentRoomAdmins = doc.data().admins || [doc.data().creatorId]; const imc = (auth.currentUser.uid === doc.data().creatorId); if(m.length === 0) { c.innerHTML = "No members"; return; }
    Promise.all(m.map(uid => db.collection('users').doc(uid).get().catch(e => null))).then(docs => {
      let html = ''; docs.forEach((d, i) => { if(!d) return; const uid=m[i], n=d.exists?d.data().displayName:"Unknown", ima=currentRoomAdmins.includes(uid); let b="";
        if (uid !== auth.currentUser.uid) { if (imc) { b += ima ? `<button onclick="removeAdminRole('${uid}')" style="background:#ffc107; border:none; padding:3px 8px; font-size:11px;">Remove Admin</button> ` : `<button onclick="makeAdmin('${uid}')" style="background:#28a745; color:white; border:none; padding:3px 8px; font-size:11px;">Make Admin</button> `; b += `<button onclick="removeMember('${uid}')" style="background:#dc3545; color:white; border:none; padding:3px 8px; font-size:11px;">Kick</button>`; } else if (currentRoomAdmins.includes(auth.currentUser.uid) && !ima) { b += `<button onclick="removeMember('${uid}')" style="background:#dc3545; color:white; border:none; padding:3px 8px; font-size:11px;">Kick</button>`; } }
        let r = (uid === doc.data().creatorId) ? '👑' : (ima ? '🛡️' : '👤'); html += `<div style="display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px solid var(--border-color);"><span style="color:var(--primary-btn); cursor:pointer;" onclick="viewUserProfile('${uid}')">${r} ${n}</span><div>${b}</div></div>`;
      }); c.innerHTML = html;
    });
  });
}
...
function editQuestion(qId) {
  db.collection('rooms').doc(currentRoomId).collection('questions').doc(qId).get().then(doc => {
    ...
    document.getElementById('addQuestionBoxTitle').innerText = "✏️ Edit Question"; 
    document.getElementById('addMoreBtnContainer').style.display = 'none';
    document.getElementById('updateQuestionBtn').style.display = 'block';
    document.getElementById('addQuestionBox').style.display = 'flex';
  });
}
...
// 🌟 UPDATE 3.5: Modified save function to handle Save & Next
function saveQuestionToFirebase(isAddNext) {
  ...
  if (editingQuestionId) { 
    db.collection('rooms').doc(currentRoomId).collection('questions').doc(editingQuestionId).update(qData).then(() => { alert("Updated!"); closeManualAddBox(); loadRoomQuestions(); });
  } else { 
    db.collection('users').doc(auth.currentUser.uid).get().then(userDoc => { 
      qData.creatorName = userDoc.exists ? userDoc.data().displayName : "Unknown"; qData.creatorUid = auth.currentUser.uid; qData.createdAt = firebase.firestore.FieldValue.serverTimestamp(); 
      db.collection('rooms').doc(currentRoomId).collection('questions').add(qData).then(() => { 
        if(isAddNext) {
          // Clear only question fields, keep topic and time
          ['queText','optA','optB','optC','optD','correctOpt'].forEach(id => document.getElementById(id).value = '');
          document.getElementById('queText').focus();
          loadRoomQuestions(); // Load in background
        } else {
          alert("Added Successfully!"); closeManualAddBox(); loadRoomQuestions(); 
        }
      }); 
    }); 
  }
...
function uploadCSV() {
  const f = document.getElementById('csv-file').files[0]; if (!f) return alert("Select CSV!"); const ti = document.getElementById('csvTopic'), ct = ti && ti.value.trim() ? ti.value.trim() : 'General';
  const r = new FileReader(); r.onload = function(e) {
    const rows = e.target.result.split('\n'); let c = 0;
    db.collection('users').doc(auth.currentUser.uid).get().then(doc => {
      const n = doc.exists ? doc.data().displayName : "Unknown";
      for (let i = 1; i < rows.length; i++) { const cols = rows[i].trim().split(','); if (cols.length >= 6) { db.collection('rooms').doc(currentRoomId).collection('questions').add({ topic: ct, question: cols[0].trim(), optionA: cols[1].trim(), optionB: cols[2].trim(), optionC: cols[3].trim(), optionD: cols[4].trim(), correct: cols[5].trim().toUpperCase(), creatorName: n, creatorUid: auth.currentUser.uid, createdAt: firebase.firestore.FieldValue.serverTimestamp() }); c++; } }
      alert(`Uploaded ${c} questions!`); document.getElementById('csv-file').value = ""; closeCsvUploadBox(); loadRoomQuestions(); 
    });
  }; r.readAsText(f);
}

function toggleSelectAll() { const c = document.getElementById('selectAllCheckbox').checked; document.querySelectorAll('.bulk-delete-chk').forEach(chk => chk.checked = c); }
function deleteSelectedQuestions() {
  const chks = document.querySelectorAll('.bulk-delete-chk:checked'); if (chks.length === 0) return alert("Select at least one.");
  if(confirm(`Delete ${chks.length} questions?`)) { let p = []; chks.forEach(chk => { p.push(db.collection('rooms').doc(currentRoomId).collection('questions').doc(chk.value).delete()); }); Promise.all(p).then(() => { let s = document.getElementById('selectAllCheckbox'); if(s) s.checked = false; loadRoomQuestions(); }); }
}
...
function loadRoomQuestions() {
  ...
  db.collection('rooms').doc(currentRoomId).collection('questions').orderBy('createdAt', 'desc').get().then(snap => {
    if (snap.empty) { c.innerHTML = 'No questions found.'; let b = document.getElementById('bulkDeleteContainer'); if(b) b.style.display = 'none'; return; }
    let aMap = {}; allCurrentQuestions = [];
    snap.forEach(doc => { let q=doc.data(); q.id=doc.id; q.topic=q.topic||'General'; allCurrentQuestions.push(q); if(attemptedList.includes(q.id)) return; let author=q.creatorName||"Unknown"; if(!aMap[author]) aMap[author]=[]; aMap[author].push(q); });
    const im = currentRoomAdmins.includes(auth.currentUser.uid); let b = document.getElementById('bulkDeleteContainer'); if(b) b.style.display = (im && allCurrentQuestions.length > 0) ? 'flex' : 'none';
    if(Object.keys(aMap).length === 0) { c.innerHTML = '<p style="color:#28a745; font-weight:bold;">🎉 All questions attempted!</p>'; return; }
    let html = ''; for(let a in aMap) { let id = `author-section-${a.replace(/\s+/g, '_')}`, open = openAuthorFolders.includes(id) ? 'block' : 'none'; html += `<div class="q-card"><h4 style="color:var(--primary-btn); cursor:pointer; font-size:15px; margin:0;" onclick="toggleAuthorQuestions('${id}')">📁 MCQ by ${a} (${aMap[a].length}) 🔽</h4><div id="${id}" style="display: ${open}; margin-top:10px;">${renderQuestionsHTML(aMap[a], im)}</div></div>`; } c.innerHTML = html;
  });
}
...
function renderQuestionsHTML(arr, im) {
  let html = ''; arr.forEach((q, i) => {
    ...
    let chk = (q.creatorUid === auth.currentUser.uid || im) ? `<input type="checkbox" class="bulk-delete-chk" value="${q.id}" style="transform:scale(1.2); margin-right:10px; accent-color:#dc3545;" onmousedown="event.stopPropagation()" ontouchstart="event.stopPropagation()">` : '';
    let opt = [{text: q.optionA, let: 'A'}, {text: q.optionB, let: 'B'}, {text: q.optionC, let: 'C'}, {text: q.optionD, let: 'D'}]; opt.sort(() => Math.random() - 0.5); 
    let oh = ''; opt.forEach(o => { oh += `<button id="btn-${q.id}-${o.let}" class="quiz-opt-btn" onmousedown="event.stopPropagation()" ontouchstart="event.stopPropagation()" onclick="checkAnswer('${q.id}', '${o.let}', '${q.correct}')">${o.text}</button>`; });
    ...
    html += `<div id="q-card-${q.id}" class="q-card" style="border:1px solid var(--border-color); margin-bottom:10px; cursor:pointer;" ${lp}><p style="font-weight:bold; margin-bottom:10px; display:flex; align-items:flex-start;">${chk} <span>Q${i+1}. ${q.question} <span class="topic-badge">${q.topic}</span> ${t}</span></p><div id="mcq-options-${q.id}" style="display:flex; flex-direction:column; gap:5px;">${oh}</div><p id="feedback-${q.id}" style="margin-top:10px; font-size:13px; font-weight:bold; display:none;"></p></div>`;
  }); return html;
}

function checkAnswer(qId, sOpt, cOpt) {
  ...
  if (sOpt === cOpt) { cb.style.background = "#d4edda"; cb.style.color = "#155724"; cb.style.borderColor = "#28a745"; fb.innerText = "✅ Correct!"; fb.style.color = "#28a745"; updateLeaderboardScore(); let a = JSON.parse(localStorage.getItem(`attempted_${currentRoomId}`)) || []; if(!a.includes(qId)) { a.push(qId); localStorage.setItem(`attempted_${currentRoomId}`, JSON.stringify(a)); } setTimeout(() => { let c = document.getElementById(`q-card-${qId}`); if(c) c.style.display = 'none'; }, 1500); } 
  else { cb.style.background = "#f8d7da"; cb.style.color = "#721c24"; cb.style.borderColor = "#dc3545"; document.getElementById(`btn-${qId}-${cOpt}`).style.background = "#d4edda"; document.getElementById(`btn-${qId}-${cOpt}`).style.borderColor = "#28a745"; document.getElementById(`btn-${qId}-${cOpt}`).style.color = "#155724"; fb.innerText = "❌ Wrong! (Saved to Revision)"; fb.style.color = "#dc3545"; let ce = document.getElementById(`q-card-${qId}`); let cc = ce.cloneNode(true); if(cc.children[0].tagName === 'DIV') cc.children[0].style.display = 'none'; if (!wrongQuestions.some(item => item.id === qId)) { wrongQuestions.push({ id: qId, html: cc.innerHTML }); localStorage.setItem('studyRoomWrong', JSON.stringify(wrongQuestions)); updateRevisionCount(); } }
